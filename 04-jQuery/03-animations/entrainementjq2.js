...
$(function(){
    ...
    $('#fadein').click(function(){
        ...
        $('#vert').fadeIn(1000).delay(1000).fadeOut(1000, finAnim); // on peut enchaîner les unes à la suite des autres. On appelle la fonction callback finAnim à la fin du fadeOut(). Attention : pas de () sinon elle s'exécute sans attendre l'ordre du fadeOut.
    });
    ...
    $('#anim1').click(function(){
        $('#un').animate({ top : '-200px'}, 1000);  // les propriétés CSS se mettent dans un objet en accolades (en entre guillemets si elles contiennent un tiret). -200px est exprimé par rapport à la position d'origine de l'élément. Notez que l'on n'est pas obligé de préciser "px" car c'est l'unité par défaut
    });

    // Animation 2 : 2 propriétés CSS dans la même fonction :
    ...
    function anim4() {
        $('#quatre').animate({ left : '400px' }, 3000)
                    .animate({ left : '0px'}, 3000, anim4); //0px signifie 0 par rapport à la position d'origine = revient à gauche du navigateur. On aurait pu mettre -=400px à la place. On peut mettre une fonction callback dans animate(), elle s'exécute une fois dans l'animation complètement terminée. Ainsi, ici on appelle la fonction elle-même à chaque fin d'animation.
    }
