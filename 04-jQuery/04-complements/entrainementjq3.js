$(function(){
    ...
    // au clic sur un <h3> qui s'appelle "effacer galerie...", la galerie qui suit s'efface : 

    $('#galeries h3').click(function(){
        ///$(this).next().fadeOut(1000);   // la méthode next() permet de sélectionner la balise directement suivante, ici le <div.galerie> qui suit le <h3> sur lequel on a cliqué. Ainsi, quelque soit le <h3> cliqué, seul la balise qui le suit s'efface. next() peut prendre un sélecteur en argument pour ne sélectionner que les balises suivantes répondant à celui-ci.

        //  Cependant, on voudrait effacer les <img> contenus dans les <div.galerie> :
    ...
    // Exercice "accordéon" : vous rendez fonctionnel l'accordéon du HTML : au clic sur "section 1" ou "section 2" ou "section 3" le <div> qu'on quitte se ferme, alors que le <div> de la nouvelle section s'ouvre. utilisez la classe "on" pour ouvrir un élément, et la classe "off" pour fermer un élément.
    ...
    var autreElementDOM = autreObjetJquery.get();   // get() transforme un objet jQuery en un array contenant tout les éléments du DOM
    console.log(autreElementDOM);   // on voit cet array

    autreElementDOM[1].addEventListener('click', function(){    // dans l'array, on cible un élément en mettant un indice en crochets 
        alert('Ce <div> est devenu un élément du DOM sur lequel on applique un syntaxe propre à JS');
    });
